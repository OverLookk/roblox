...
class ESPOverlay:
    ...
    def draw_fov_ring(self, x, y, radius, color):
        # Pyglet Y is bottom-up, input Y is top-down
        screen_y = self.screen_h - y
        if len(color) == 3: color = (*color, 255)
        
        # shapes.Arc is used for outlines (Available in Pyglet 1.5.4+)
        # angle=360 creates a full circle outline
        try:
            arc = shapes.Arc(x, screen_y, radius, angle=360, color=color, batch=self.batch)
            self.active_elements.append(arc)
        except AttributeError:
            # Fallback if Arc is not available (older pyglet versions)
            pass
    ...
    def render(self):
        ...
        # Update Window Info first (needed for FOV Ring center)
        ...
        for p in players:
            # TEAM CHECK: Skip rendering if same team
