...
class ESP_GUI:
    def __init__(self, root, memory_ref):
        ...
        self.root.title("ovrhax - @obfuscateme")
        self.root.geometry("750x610") # Increased height for new slider
        ...
        self.build_system_section(self.left_col)
        ...
        try:
            ...
            if getattr(widget, "is_panel", False):
                ...
                if getattr(widget, "is_color_btn", False):
                    widget.configure(highlightbackground=accent, highlightcolor=accent, highlightthickness=1, bd=1, relief="solid")
                else:
    ...
    def build_theme_section(self, parent):
        ...
        self.btn_bg_col = tk.Button(row_bg, bg=visuals.UI_BG_COLOR, width=8, relief="flat", command=lambda: self.pick_ui_color("UI_BG_COLOR", self.btn_bg_col))
        ...
        self.btn_acc_col = tk.Button(row_acc, bg=visuals.UI_ACCENT_COLOR, width=8, relief="flat", command=lambda: self.pick_ui_color("UI_ACCENT_COLOR", self.btn_acc_col))
        ...
        self.btn_pnl_col = tk.Button(row_pnl, bg=visuals.UI_PANEL_COLOR, width=8, relief="flat", command=lambda: self.pick_ui_color("UI_PANEL_COLOR", self.btn_pnl_col))
    ...
    def build_esp_section(self, parent):
        ...
        frame = self.create_panel(parent)
        self.var_esp = self.create_checkbox(frame, "Enable", visuals.ENABLE_ESP)
        ...
        default_dist = getattr(visuals, "ENABLE_DISTANCE_TEXT", False) 
        self.var_dist_text = self.create_checkbox(frame, "Distance", default_dist)
    ...
    def build_aimbot_section(self, parent):
        ...
        self.btn_fov_col = tk.Button(row_fov_color, bg=self.rgb_to_hex(visuals.FOV_COLOR), width=8, relief="flat", command=lambda: self.pick_color(self.btn_fov_col, "FOV_COLOR"))
        ...
        self.combo_priority.bind("<<ComboboxSelected>>", self.update_visuals)
        ...
        self.combo_aim_mode.bind("<<ComboboxSelected>>", self.update_visuals)
    ...
    def build_trigger_section(self, parent):
        ...
        row_type.is_panel = True
        row_type.pack(fill='x', padx=10, pady=(4, 2))
        tk.Label(row_type, text="Type:", font=("Segoe UI", 8)).pack(anchor='w')
        self.combo_trig_type = ttk.Combobox(row_type, values=["Click", "Hold", "Spam"], state="readonly")
        self.combo_trig_type.set(getattr(visuals, "TRIGGER_TYPE", "Click")) 
        self.combo_trig_type.pack(fill='x', expand=True, pady=(2, 0))
        self.combo_trig_type.bind("<<ComboboxSelected>>", self.update_visuals)

        row_mode = tk.Frame(frame)
        row_mode.is_panel = True
        row_mode.pack(fill='x', padx=10, pady=(4, 2))
        tk.Label(row_mode, text="Mode:", font=("Segoe UI", 8)).pack(anchor='w')
        trig_modes = ["Hold", "Toggle"]
        if visuals.TRIGGER_MODE not in trig_modes: visuals.TRIGGER_MODE = "Hold"
        self.combo_trig_mode = ttk.Combobox(row_mode, values=trig_modes, state="readonly")
        ...
        default_spam = getattr(visuals, "TRIGGER_SPAM_DELAY", 100)
        self.slider_spam_speed, self.lbl_spam_speed = self.create_labeled_slider(frame, "Spam (ms):", 50, 200, default_spam)
        
        # --- NEW TOLERANCE SLIDER ---
        default_tolerance = getattr(visuals, "TRIGGER_TOLERANCE", 1.0)
        self.slider_trig_tolerance, self.lbl_trig_tolerance = self.create_labeled_slider(frame, "Tolerance:", 0.1, 3.0, default_tolerance, is_float=True)
    ...
    def build_colors_section(self, parent):
        ...
        self.btn_tracer_col = tk.Button(row_tracer, bg=self.rgb_to_hex(visuals.TRACER_COLOR), width=8, relief="flat", command=lambda: self.pick_color(self.btn_tracer_col, "TRACER_COLOR"))
        ...
        self.btn_dot_col = tk.Button(row_dot, bg=self.rgb_to_hex(visuals.DOT_COLOR), width=8, relief="flat", command=lambda: self.pick_color(self.btn_dot_col, "DOT_COLOR"))
        self.btn_dot_col.pack(side='right', padx=5)
        self.btn_dot_col.is_color_btn = True

    def build_system_section(self, parent):
        self.create_section_header(parent, "System")
        frame = self.create_panel(parent)
        frame.pack_configure(pady=(6, 10))
        btn_grid = tk.Frame(frame)
        btn_grid.is_panel = True
        ...
        btn_att = tk.Button(btn_grid, text="RE-ATTACH", bg="#2979FF", fg="white", font=("Segoe UI", 8, "bold"), relief="flat", command=self.reattach_game)
        btn_att.grid(row=0, column=0, padx=2, sticky="ew")
        btn_att.is_special_btn = True
        btn_save = tk.Button(btn_grid, text="SAVE", bg="#00C853", fg="white", font=("Segoe UI", 8, "bold"), relief="flat", command=visuals.save)
        btn_save.grid(row=0, column=1, padx=2, sticky="ew")
        btn_save.is_special_btn = True
        btn_exit = tk.Button(btn_grid, text="EXIT", bg="#D50000", fg="white", font=("Segoe UI", 8, "bold"), relief="flat", command=self.close_app)
    ...
    def format_key_name(self, key_code):
        mouse_map = {0x01: "L-CLICK", 0x02: "R-CLICK", 0x04: "M-CLICK", 0x05: "X1", 0x06: "X2"}
    ...
    def update_visuals(self, *args):
        ...
        visuals.ENABLE_DISTANCE_TEXT = self.var_dist_text.get()
        ...
        visuals.TRIGGER_TYPE = self.combo_trig_type.get() 
        visuals.TRIGGER_MODE = self.combo_trig_mode.get() 
        visuals.TRIGGER_DELAY = float(self.slider_trig_delay.get())
        visuals.TRIGGER_SPAM_DELAY = int(self.slider_spam_speed.get()) 
        visuals.MAX_DISTANCE = int(self.slider_dist.get())
        
        # New Update
        visuals.TRIGGER_TOLERANCE = float(self.slider_trig_tolerance.get())
        ...
        self.lbl_spam_speed.config(text=str(visuals.TRIGGER_SPAM_DELAY))
        self.lbl_trig_tolerance.config(text=f"{visuals.TRIGGER_TOLERANCE:.1f}")
